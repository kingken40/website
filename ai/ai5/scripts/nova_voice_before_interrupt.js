...
// nova custom audio-pack voices (WAV-based, separate from browser TTS voices)
const nova_AUDIO_PACKS = [
    { id: 'nova-pack-cfx', name: 'nova Pack - CFX', basePath: '../nova/CFX_UI_J.A.R.V.I.S' },
    { id: 'nova-pack-ghv4', name: 'nova Pack - GHV4', basePath: '../nova/GHV4_UI_J.A.R.V.I.S' },
    { id: 'nova-pack-proffie', name: 'nova Pack - ProffieOS V2', basePath: '../nova/ProffieOS_V2_Voicepack_J.A.R.V.I.S' },
    { id: 'nova-pack-sn4', name: 'nova Pack - SN4', basePath: '../nova/SN4_UI_J.A.R.V.I.S' },
    { id: 'nova-pack-xeno2', name: 'nova Pack - Xeno2', basePath: '../nova/Xeno2_UI_J.A.R.V.I.S' },
    { id: 'nova-pack-xeno3', name: 'nova Pack - Xeno3', basePath: '../nova/Xeno3_UI_J.A.R.V.I.S' }
];

let selectedVoiceMode = 'tts'; // 'tts' | 'nova-pack'
let selectednovaPackId = null;
let currentPackAudio = null;
let lastnovaPackClipByPack = {};
...
function ensurenovaResponseVoice() {
    ...
    if (isUsableSpeechVoice(bestVoice)) {
        ...
        console.log('🎯 nova response TTS voice set to:', bestVoice.name, `(${bestVoice.lang})`);
        return bestVoice;
    }

    console.warn('🎯 No usable nova response TTS voice available');
    return null;
}
...
async function playViaLocalVoiceBridge(text, onEndCallback) {
    ...
    try {
        ...
        if (now - lastBridgeFailureNoticeMs > 6000) {
            ...
            showVoiceNotification('Local nova bridge unavailable. Start Start_Voice_Bridge.bat in nova-main.', 4500);
        }
...
function getNovaLikeVoices(voices) {
    ...
    const NovaVoicePatterns = [
        // nova-specific voices (Paul Bettany style - highest priority)
        { pattern: /liam.*(brit|united kingdom|uk|gb)/i, score: 115, name: 'Liam (British nova)' },
        { pattern: /paul.*(brit|united kingdom|uk|gb)/i, score: 114, name: 'Paul (British nova)' },
        ...
        { pattern: /liam/i, score: 105, name: 'Liam (nova Voice)' },
    ...
    voices.forEach(voice => {
        ...
        if (voice.name.toLowerCase().includes('liam') || voice.lang.includes('CA')) {
            bestScore += 12; // Canadian (Liam) accent bonus for nova voice
        }
...
function speakText(text, onEndCallback = null) {
    ...
    console.log('🔊 selectedVoiceMode:', selectedVoiceMode, 'selectednovaPackId:', selectednovaPackId);
    ...
    const startSpeaking = async () => {
        ...
        if (selectedVoiceMode === 'nova-pack' && selectednovaPackId) {
            ensurenovaResponseVoice();
        }
...
function selectVoiceById(voiceIndex) {
    if (window.availableNovaVoices && window.availableNovaVoices[voiceIndex]) {
        ...
        localStorage.removeItem('Nova_selected_nova_pack');
...
function createVoiceSamplePhrase() {
    if (typeof window.getRandomnovaStylePhrase === 'function') {
        const dynamicPhrase = window.getRandomnovaStylePhrase();
...
function populateVoiceSelection() {
    ...
    const novaPacks = Array.isArray(window.nova_AUDIO_PACKS) ? window.nova_AUDIO_PACKS : [];
    if (novaPacks.length > 0) {
        const packSeparator = document.createElement('option');
        packSeparator.value = '';
        packSeparator.textContent = '───── nova Audio Packs (.wav) ─────';
        ...
        novaPacks.forEach(pack => {
            ...
            if (selectedVoiceMode === 'nova-pack' && selectednovaPackId === pack.id) {
    ...
    availableVoices.forEach((voice, index) => {
        ...
        if (selectedVoiceMode !== 'nova-pack' && currentVoiceSettings.voice && voice.name === currentVoiceSettings.voice.name) {
            option.selected = true;
        }
        voiceSelect.appendChild(option);
    });
    ...
    if (selectedVoiceMode === 'nova-pack' && selectednovaPackId) {
        const packOption = `pack:${selectednovaPackId}`;
    ...
    if (voiceSelect) {
        voiceSelect.addEventListener('change', function() {
            ...
            if (selectedValue.startsWith('pack:')) {
                ...
                ensurenovaResponseVoice();
                const played = playnovaPackSample('Voice pack selected, sir.');
    ...
    if (previewBtn) {
        previewBtn.addEventListener('click', function() {
            ...
            if (selectedValue && selectedValue.startsWith('pack:')) {
                ...
                const played = playnovaPackSample(sampleText);
...
function getSelectednovaPack() {
    if (!selectednovaPackId) return null;
    return nova_AUDIO_PACKS.find(p => p.id === selectednovaPackId) || null;
}
...
function choosenovaPackClip(pack, text = '') {
    ...
    const lastPlayedClip = lastnovaPackClipByPack[packId];
...
function playnovaPackSample(text = '', onEndCallback = null) {
    const pack = getSelectednovaPack();
    if (!pack) return false;

    const clipCandidates = choosenovaPackClip(pack, text);
    let candidateIndex = 0;

    const tryPlayNext = () => {
        if (candidateIndex >= clipCandidates.length) {
            console.warn('🔊 No playable nova clip found in pack:', pack.name);
            return false;
        }
        ...
        audio.onplay = function () {
            lastnovaPackClipByPack[pack.id] = clip;
            isSpeaking = true;
            window.isSpeaking = true;
            isSpeechOutputActive = true;
            window.isSpeechOutputActive = true;
            updateSpeakingUI(true);
            console.log('🔊 Playing nova pack clip:', src);
        };
...
window.nova_AUDIO_PACKS = nova_AUDIO_PACKS;
window.setVoiceModeSelection = function(mode, packId = null) {
    selectedVoiceMode = mode === 'nova-pack' ? 'nova-pack' : 'tts';
    selectednovaPackId = selectedVoiceMode === 'nova-pack' ? packId : null;
    localStorage.setItem('Nova_voice_mode', selectedVoiceMode);
    if (selectednovaPackId) {
        localStorage.setItem('Nova_selected_nova_pack', selectednovaPackId);
        ensurenovaResponseVoice();
    } else {
        localStorage.removeItem('Nova_selected_nova_pack');
    }
    console.log('🎤 Voice mode selection updated:', selectedVoiceMode, selectednovaPackId);
};
...
window.loadVoiceModeSelection = function() {
    ...
    selectedVoiceMode = savedMode === 'nova-pack' ? 'nova-pack' : 'tts';
    selectednovaPackId = savedMode === 'nova-pack' ? savedPack : null;
    console.log('🎤 Loaded voice mode selection:', selectedVoiceMode, selectednovaPackId);
};
