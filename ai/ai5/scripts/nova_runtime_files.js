...
function formatMessageContent(text) {
    ...
    function formatMathExpression(expression) {
        let safeExpression = escapeHtml(String(expression).trim());
    ...
    // Keep mathematical notation visually separate from prose. Support both
    // Markdown-style delimiters and common plain-text formulas from models.
    formatted = formatted.replace(/\$\$([\s\S]+?)\$\$/g, (match, expression) => (
        storePlaceholder(`<div class="math-display" role="math">${formatMathExpression(expression)}</div>`)
    ));
    formatted = formatted.replace(/\\\[([\s\S]+?)\\\]/g, (match, expression) => (
        storePlaceholder(`<div class="math-display" role="math">${formatMathExpression(expression)}</div>`)
    ));
    formatted = formatted.replace(/\\\(([\s\S]+?)\\\)/g, (match, expression) => (
        storePlaceholder(`<span class="math-inline" role="math">${formatMathExpression(expression)}</span>`)
    ));
    formatted = formatted.replace(/(^|[\s:])((?:sigmoid|softmax|relu|tanh|log|ln|exp)\s*\([^)\n]+\)(?:\s*=\s*[^,.;\n]+)?)/gi, (match, prefix, expression) => (
        `${prefix}${storePlaceholder(`<span class="math-inline" role="math">${formatMathExpression(expression)}</span>`)}`
    ));
    ...
    const allowedTags = ['strong', 'em', 'code', 'br', 'div', 'span', 'sub', 'sup'];
